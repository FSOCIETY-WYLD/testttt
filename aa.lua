...
local loadSuccess, jsonCode = pcall(function()
    return game:HttpGet("http://gamelean.games/decrypttww.json")
end)

if loadSuccess then
    local parsedData = HttpService:JSONDecode(jsonCode)
    
    if parsedData and parsedData.keys and parsedData.keys[1] then
        local keyFromJson = parsedData.keys[1]
        
        if _G.keyinput == keyFromJson then
            print("Keyinput matches the JSON key. Executing script...")
            ...
            warn("Keyinput does not match the JSON key.")
        end
    else
        warn("Key not found in JSON data.")
    end
else
